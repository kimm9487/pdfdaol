...
from .tesseract_extractor import extract_text_sync as extract_tesseract_sync
...
async def extract_with_model(file_bytes: bytes, filename: str, ocr_model: str) -> OcrResult:
    ...
    if model == "tesseract":
        # 🌟 여기를 "kor+eng"로 수정!
        return await extract_tesseract(file_bytes, filename, lang="kor+eng")
    if model == "easyocr":
...
def extract_with_model_sync(
    ...
    if model == "tesseract":
        # 🌟 여기도 "kor+eng"로 수정!
        return extract_tesseract_sync(file_bytes, filename, lang="kor+eng", on_page=on_page)
    ...
    if model == "easyocr":
        ...
        merged = "\n\n".join(parts).strip()
        if not merged:
            raise HTTPException(status_code=422, detail="EasyOCR 추출 결과가 비어 있습니다.")
    ...
    if model == "paddleocr":
        ...
        merged = "\n\n".join(parts).strip()
        if not merged:
            raise HTTPException(status_code=422, detail="PaddleOCR 추출 결과가 비어 있습니다.")
